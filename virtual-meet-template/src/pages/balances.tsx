...
function BalancesPage() {
    ...
    const {nativeBalance, tokenBalances, loading, message} = useBalances();
        ...
        <main className={styles.main}>
            <div className={styles.center}>
                <div>
                    ...
                    <p className="my-8 text-center text-xl font-bold  ">
                        {'Native Balance is ' + nativeBalance?.balance}
                        <br />
                        {'Token Balance is ' + tokenBalances?.length}
                    </p>


                    <div className=" mx-4 my-8 grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                        {tokenBalances?.map((token) => (
                            <TokenCard key={token.token_address} {...token} />
                        ))}
                    </div>
